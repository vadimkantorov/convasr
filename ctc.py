...
def alignment(log_probs, targets, input_lengths, target_lengths, blank = 0):
	...
	_t_a_r_g_e_t_s_ = torch.cat([torch.stack([torch.full_like(targets, blank), targets], dim=-1).flatten(start_dim=-2),
		torch.full_like(targets[:, :1], blank)], dim=-1)
	...
	zero, zero_padding = torch.tensor(torch.finfo(log_probs.dtype).min, device=log_probs.device,
			dtype=log_probs.dtype), 2
	...
	l1l2 = log_alpha.gather(-1,
			torch.stack([zero_padding + target_lengths * 2 - 1, zero_padding + target_lengths * 2], dim=-1))
	...
	return torch.zeros_like(_t_a_r_g_e_t_s_, dtype=torch.long).scatter_(-1, (path.t() - zero_padding).clamp(min=0),
			torch.arange(len(path), device=log_alpha.device).expand(len(B), -1))[:, 1::2]
