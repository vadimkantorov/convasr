...
#finfo_min = {
#	torch.float32 : torch.finfo(torch.float32).min
...
def alignment(log_probs, targets, input_lengths, target_lengths, blank = 0, pack_backpointers = False):
	...
	padded_t = int(math.ceil(padded_t / 4)) * 4
	backpointers_shape = (len(log_probs), len(B), padded_t)
	backpointers = torch.zeros(backpointers_shape if not pack_backpointers else (backpointers_shape[:-1] + (padded_t // 4, )), device = log_probs.device, dtype = torch.uint8)
