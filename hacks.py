...
def print_memory_stats(prefix = '', scaler = dict(mb = 1e6)):
	...
	for device in range(torch.cuda.is_available and torch.cuda.device_count() or 0):
		print('MEMORY', prefix, 'reserved', torch.cuda.memory_reserved(device) / v, 'allocated', torch.cuda.memory_allocated(device) / v, k)
		print('MEMORY MAX', prefix, 'reserved', torch.cuda.max_memory_reserved(device) / v, 'allocated', torch.cuda.max_memory_allocated(device) / v, k)
