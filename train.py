...
def main(args):
	...
	if args.onnx:
		...
		model_input_casted = lambda x: model(x.to(dtype = dtype)).to(dtype = torch.float32)
		waveform_input = torch.rand(args.onnx_sample_batch_size, args.onnx_sample_time, device = args.device)
		logits = model_input_casted(waveform_input)
		torch.onnx.export(model_input_casted, (waveform_input,), args.onnx, opset_version = args.onnx_opset, export_params = args.onnx_export_params, do_constant_folding = True, input_names = ['x'], output_names = ['logits'], dynamic_axes = dict(x = {0 : 'B', 1 : 'T'}, logits = {0 : 'B', 2: 't'}))
		...
		(logits_, ) = onnxruntime.run(None, dict(x = waveform_input.cpu().numpy()))
