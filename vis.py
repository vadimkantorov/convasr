...
def errors(input_path, include = [], exclude = [], audio = False, output_path = None, sortdesc = None, topk = None, duration = None, cer = None, wer = None, mer = None, filter_transcripts = None, strip_audio_path_prefix = None):
	...
	f.write('<tr><td>&nbsp;</td></tr>')
	f.write('\n'.join(f'''<tr class="first"><td colspan="6">''' + (f'<audio controls src="{audio_data_uri(utt[0]["audio_path"][len(strip_audio_prefix):])}"></audio>' if audio else '') + f'<div class="nowrap">{utt[0]["audio_name"]}</div></td><td>{word_alignment(utt[0], ref = True, flat = True)}</td><td>{word_alignment(utt[0], ref = True, flat = True)}</td></tr>' + '\n'.join(f'<tr class="any"><td class="br">{transcripts.audio_name(input_path[i])}</td><td>{a.get("cer_easy", 1):.02%}</td><td>{a.get("cer", 1):.02%}</td><td>{a.get("wer_easy", 1):.02%}</td><td>{a.get("wer", 1):.02%}</td><td class="br">{a.get("mer", 1):.02%}</td><td>{word_alignment(a.get("words", []))}</td><td>{word_alignment(a, hyp = True, flat = True)}</td></tr>' for i, a in enumerate(utt)) for utt in cat))
...
def tabulate(experiments_dir, experiment_id, entropy, loss, cer10, cer15, cer20, cer30, cer40, cer50, per, wer, json_, bpe, der, lang):
