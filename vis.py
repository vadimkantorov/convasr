...
def errors(input_path, audio_name = None, audio = False, output_file_name = None, sortdesc = None):
	...
	open(output_file_name , 'w').write('<html><meta charset="utf-8"><style>.br{border-right:2px black solid} tr.first>td {border-top: 1px solid black} tr.any>td {border-top: 1px dashed black}  .nowrap{white-space:nowrap}</style><body><table style="border-collapse:collapse; width: 100%"><tr><th></th><th>cer</th><th>wer</th><th>mer</th><th></th></tr>' + '\n'.join(f'''<tr class="first"><td>''' + (f'<audio controls src="data:audio/wav;base64,{base64.b64encode(open(utt[0]["audio_path"], "rb").read()).decode()}"></audio>' if audio else '') + f'<div class="nowrap">{utt[0]["audio_name"]}</div></td></td><td></td><td></td><td></td><td>{colorize_alignment(utt[0], ref = True, flat = True)}</td><td>{colorize_alignment(utt[0], ref = True, flat = True)}</td></tr>' + '\n'.join(f'<tr class="any"><td class="br">{os.path.basename(input_path[i])}</td><td>{a["cer"]:.02%}</td><td>{a["wer"]:.02%}</td><td class="br">{a["mer"]:.02%}</td><td>{colorize_alignment(a["words"])}</td><td>{colorize_alignment(a, hyp = True, flat = True)}</td></tr>' for i, a in enumerate(utt)) for utt in cat)   + '</table></body></html>')
...
def colorize_alignment(transcript, ref = None, hyp = None, flat = False, tag = '<pre>', prefix = True):
