...
def dump(by_group, splits, subset_name, gz = True):
	for split_name, transcript in by_group.items():
		...
		print(
			...
			len(transcript) // 1000,
			'K utt |',
			int(sum(transcripts.compute_duration(t, hours = True) for t in transcript)),
			'hours'
		)
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'--exclude', nargs = '*', default = ['public_exclude_file_v5.csv.gz', 'exclude_df_youtube_1120.csv.gz']
	)
	...
	transcript = [
		dict(
			audio_path = s[-1],
			audio_name = os.path.basename(s[-1]),
			group = s[2],
			begin = 0.0,
			end = float(s[3]),
			ref = s[-3],
			file_size_kb = float(s[5])
		) for i,
		l in enumerate(gzopen(args.metadata)) if not is_header_line(i, l) for s in [l.strip().split(',')]
	]
	exclude = set(
		os.path.basename(audio_path) for f in args.exclude for i,
		l in enumerate(gzopen(f)) if not is_header_line(i, l) for s in [l.split(',')] for audio_path in [s[1]]
	)
	filtered_by_cer = set(
		os.path.basename(audio_path) for i,
		l in enumerate(gzopen(args.benchmark)) if not is_header_line(i, l) for s in [l.strip().split(',')]
		for audio_path,
		group,
		cer in [(s[1], s[-1], float(s[-3]))] if cer <= args.max_cer[group]
	)
	transcript = list(
		filter(
			lambda t: t.pop('file_size_kb') >= args.min_kb and t['audio_name'] not in exclude and
			('_val' in t['group'] or t['audio_name'] in filtered_by_cer),
			transcript
		)
	)
	...
	by_group = {
		k: list(g)
		for k,
		g in itertools.groupby(sorted(transcript, key = lambda t: t['group']), key = lambda t: t['group'])
	}

	clean = split(
		by_group, ['voxforge_ru', 'ru_RU', 'russian_single', 'public_lecture_1', 'public_series_1'],
		dict(train = 0.95, val = 0.05)
	)

	mixed_ = split(by_group, ['buriy_audiobooks_2_val', 'public_youtube700_val'], dict(val = None))
	mixed = split(
		by_group,
		['private_buriy_audiobooks_2', 'public_youtube700', 'public_youtube1120', 'public_youtube1120_hq', 'radio_2'],
		dict(train = None)
	)
