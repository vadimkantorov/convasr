...
def main(args):
	...
	for i, info_path in enumerate(glob.glob(os.path.join(args.input_path, '*.json'))):
		try:
			...
			transcripts_ = [
				dict(audio_path = info_path.replace('.json', ''), speaker_name = "", **{k: v
																		for k, v in t.items()
																		if k not in args.strip})
	...
	for t in transcripts:
		t['ref'] = clean_up_transcript_ref(t['ref'])
	...
	if args.split_by_parts:
		...
		for i in range(args.split_by_parts):
			json.dump(
				transcripts[i * step:(i + 1) * step],
				open(args.output_path.replace('.json', '') + f'{i}' + '.json', 'w'),
				ensure_ascii = False,
				indent = 2,
				sort_keys = True
			)
