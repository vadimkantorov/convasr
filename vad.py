...
def detect_speech(signal, sample_rate, window_size, aggressiveness, window_size_dilate = None):
	...
	speech = torch.as_tensor([[
		len(chunk) == frame_len and vad.is_speech(bytearray(chunk.numpy()), sample_rate)
...
def upsample(speech, log_probs):
	return F.interpolate(
		speech[:, None, :].to(device = log_probs.device, dtype = torch.float32), (log_probs.shape[-1], )
	).squeeze(1).round().bool()
