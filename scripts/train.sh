python3 train.py $@ \
  ...
  --verbose --lang ru \
  --model JasperNetBig \
  --train-batch-size 4 --val-batch-size 256 \
  --scheduler MultiStepLR --decay-milestones 25000 35000 \
  --lr 1e-3 \
  --optimizer NovoGrad \
  --train-data-path data/trainset20122019.csv \
  --val-data-path data/mixed_val.csv data/clean_val.csv data/valset20122019.0.csv data/valset20122019.1.csv data/valset20122019.csv \
  --analyze kontur_calls_micro.csv \
  --val-iteration-interval 500 \
  --epochs 23 \
  --bpe data/tts_dataset_bi_200.model \
  --experiment-name bpe_bi200_conv1d_epilogue40_finetune_decays_newval_freeze6_lr001 \
  --bpe data/tts_dataset_bi_200.model \
  --fp16 O2 \
