...
TRANSCRIBE='--mono --join-transcript --batch-time-padding-multiple 1 --align --skip-processed --max-segment-duration 4.0 --transcribe-first-n-sec 1800 --text-pipelines dataset'
...
H_1000_SPLIT='--microval-duration-in-hours 0 --val-duration-in-hours 1000 --test-duration-in-hours 100 --do-not-save-remains '
...
# if there are any empty ref if cut.json then remove it with cleanup_transcripts_with_empty_ref tool
# python tools.py cleanup_transcripts_with_empty_ref --input-path youtube_v12/cut/cut.json
...
# python tools.py filter_dataset --input-path youtube_v12/cut/cut_train.json --output-path youtube_v12/cut/cut_train_cer_25_1000h.json --duration-in-hours 1000 --cer 25
